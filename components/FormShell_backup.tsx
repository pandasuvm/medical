...
export default function FormShell() {
  ...
  const phases = [
    {
      ...
      requiredFields: ['firstName', 'lastName', 'age', 'weight', 'height']
    },
    {
      id: 'vitals',
      title: 'Vital Signs',
      ...
      requiredFields: ['heartRate', 'systolicBP', 'diastolicBP', 'respiratoryRate', 'spo2']
    },
    {
      id: 'gcs',
      ...
      requiredFields: ['eyeOpening', 'verbalResponse', 'motorResponse']
    },
    {
      id: 'comorbidities',
      title: 'Comorbidities',
    ...
    },
    {
      ...
      requiredFields: ['primary', 'urgency']
    },
    {
      id: 'leon',
      title: 'LEON Score',
      ...
      requiredFields: ['largeTongue', 'thyroMentalDistance', 'obstruction', 'neckMobility']
    },
    {
      id: 'monitoring',
      title: 'Monitoring',
      ...
      requiredFields: ['capnography', 'arterialLine']
    }
  ];
  ...
  // Check if a phase is completed based on required fields
  const isPhaseCompleted = (phaseId: string, formData: any) => {
    const phase = phases.find(p => p.id === phaseId);
    if (!phase || !phase.requiredFields.length) return true;

    const phaseData = formData[phaseId] || {};
    return phase.requiredFields.every(field => {
  ...
  // Calculate completion status for all phases
  const phaseCompletionStatus = phases.map((phase, index) => ({
    ...phase,
    isCompleted: index < currentPhaseIndex || isPhaseCompleted(phase.id, currentFormData),
    isAccessible: index <= currentPhaseIndex || isPhaseCompleted(phases[index - 1]?.id, currentFormData)
  }));
                ...
                {phases.map((phase, index) => {
                  ...
                  return (
                    <button
                      ...
                      className={`
                        ${isActive ? 'desktop-nav-item-active' : 'desktop-nav-item'}
                        ${isCompleted ? 'desktop-nav-item-completed' : ''}
                      `}
                    >
                      <div className={`
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <div className="font-medium text-sm">{phase.title}</div>
                        <div className="text-xs text-gray-500 line-clamp-1">{phase.description}</div>
                      </div>
                      {isCompleted && (
